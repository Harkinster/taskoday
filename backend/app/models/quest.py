...
class Quest(Base):
    ...
    # Optional specific day for a one-off optional quest.
    scheduled_date: Mapped[Optional[date]] = mapped_column(Date, nullable=True, index=True)
    points_reward: Mapped[int] = mapped_column(Integer, default=3, nullable=False)
    ...
    child_profile = relationship("ChildProfile", back_populates="quests")
    completions = relationship("TaskCompletion", back_populates="quest", cascade="all, delete-orphan")
